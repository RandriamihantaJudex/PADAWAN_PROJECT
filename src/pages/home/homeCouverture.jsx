...
export const MiniNavMobile = ({ disapear }) => {
    return <div className="maxIndex h-11 w-full bg-[#008F64] px-8 flex items-center justify-between text-white md:hidden fixed">
        ...
        <FontAwesomeIcon icon={faBars} onClick={(e) => disapear()} className="h-5 w-5 cursor-pointer menuAnimate "/>
    </div>
}
...
function Couverture() {
    ...
    const showPower=(e)=>{
        presentationRef.current.classList.remove('hidden')
        presentationRef.current.classList.remove('exitSlide')
        presentationRef.current.classList.add('powerSlide')
        e.target.innerHTML=`C'EST PARTIS , GO!!`
    }
    const noShowPower=(e,texte)=>{
        e.target.innerHTML=texte
        presentationRef.current.classList.remove('retour')
        presentationRef.current.classList.remove('hidden')
        presentationRef.current.classList.add('exitSlide')
    }
    return (
        <div>
            <div ref={presentationRef} className="fixed h-[100vh] w-4/12 bg-[#008F64] z-40 hidden"></div>
            {/* PARTIE GAUCHE  */}
            <div className="h-screen w-full flex justify-between greece ">
                ...
                <div className="h-full w-full flex flex-col items-center">
                    ...
                    <div className="flex">
                        <Link onMouseEnter={(e)=>showPower(e)} onMouseLeave={(e)=>noShowPower(e,'DEVENIR MENTOR')} className="z-50 mx-3 px-8 py-2.5 rounded-lg text-[#008F64] border-2 transition border-[#008F64] hover:bg-[#008F64] hover:text-white max-md:text-[12px] max-md:rounded-md max-md:font-thin max-md:border max-md:px-4 max-md:lowercase max-md:py-[2px] centrer ">
                            DEVENIR MENTOR
                        </Link>
                        <Link onMouseEnter={(e)=>showPower(e)} onMouseLeave={(e)=>noShowPower(e,'CHERCHER UN MENTOR')} className="mx-3 centrer bg-[#008F64] text-white px-8 py-2.5 rounded-lg transition hover:bg-emerald-600 hover:shadow-sm hover:shadow-[#008F64]  max-md:rounded-md  max-md:px-4 max-md:py-0 max-md:text-[12px] max-md:lowercase max-md:font-thin">
                            CHERCHER UN MENTOR
                        </Link>
                    </div>
                </div>
